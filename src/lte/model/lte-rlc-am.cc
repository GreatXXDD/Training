...
LteRlcAm::DoNotifyTxOpportunity (uint32_t bytes, uint8_t layer, uint8_t harqId, uint8_t componentCarrierId, uint16_t rnti, uint8_t lcid)
{
  ...
  NS_LOG_LOGIC ("after operate txBufferSize      = " << m_txonBufferSize );
...
LteRlcAm::DoReportBufferStatus (void)
{
  ...
  if ( r.txQueueSize != 0 || r.retxQueueSize != 0 || r.statusPduSize != 0 )
    {
      NS_LOG_INFO ("Send ReportBufferStatus: txQueueSize" << r.txQueueSize << ", txQueueHolDelay:" << r.txQueueHolDelay << ", retxQueueSize:" 
                                               << r.retxQueueSize << ", retxQueueHolDelay:" << r.retxQueueHolDelay << ",statusPduSize: " 
                                               << r.statusPduSize);
