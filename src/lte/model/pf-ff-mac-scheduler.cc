                  ...
                  if (LcActivePerFlow ((*it).first) > 0)
                    {
                      ...
                      EpsBearer bear;
                      uint8_t curqci = bear.GetPriority((*it).second.qci);
                      double curErrRate = bear.GetPacketErrorLossRate(curqci);
                      uint16_t curDelayMs = bear.GetPacketDelayBudgetMs(curqci);
                      double rcqi = log(1/curErrRate)  * curDelayMs * (achievableRate / (*it).second.lastAveragedThroughput);
                      NS_LOG_INFO (this << " RNTI " << (*it).first << " QCI " << +(*it).second.qci << " Priority " << +curqci << " PacketErrorLossRate " << curErrRate
                      << " PacketDelayBudgetMs " << curDelayMs <<" MCS " << (uint32_t)mcs << " achievableRate " << achievableRate << " avgThr " << (*it).second.lastAveragedThroughput << " RCQI " << rcqi);
                      NS_LOG_INFO (" rcqi "<<rcqi);
                      if (rcqi > rcqiMax)
                        {
                          rcqiMax = rcqi;
                          itMax = it;
  ...
  for (itStats = m_flowStatsDl.begin (); itStats != m_flowStatsDl.end (); itStats++)
    {
      ...
      NS_LOG_INFO (this <<" RNTI " << (*itStats).first << " UE total bytes " << (*itStats).second.totalBytesTransmitted);
      NS_LOG_INFO (this <<" RNTI " << (*itStats).first << " UE average throughput " << (*itStats).second.lastAveragedThroughput);
      (*itStats).second.lastTtiBytesTrasmitted = 0;
    }
