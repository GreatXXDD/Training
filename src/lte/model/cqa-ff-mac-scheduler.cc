      ...
      if (it == m_flowStatsDl.end ())
        {
          ...
          flowStatsDl.qci = params.m_logicalChannelConfigList[i].m_qci;
          NS_LOG_FUNCTION("传递到MAC的QCI"<<+flowStatsDl.qci<<"逻辑信道"<<+params.m_logicalChannelConfigList[i].m_logicalChannelIdentity);
          ...
          flowStatsUl.qci = params.m_logicalChannelConfigList[i].m_qci;
      ...
      else
        {
          ...
          NS_LOG_FUNCTION("已经存在的RNTI："<<it->first);
          it->second.qci=params.m_logicalChannelConfigList[i].m_qci;
          NS_LOG_FUNCTION("更新后的QCI"<<+it->second.qci<<"逻辑信道"<<+params.m_logicalChannelConfigList[i].m_logicalChannelIdentity);
...
CqaFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  ...
  NS_LOG_INFO(" 已经分配的RBG数量 "<<rbgAllocatedNum<<" 剩余RBG "<< m_cschedCellConfig.m_dlBandwidth / rbgSize -rbgAllocatedNum);
      ...
      while (availableRBGs.size ()>0 and itCurrentGroup->second.size ()>0)
        {
          ...
          NS_LOG_INFO (" 可以用的RBG数量 "<<availableRBGs.size ());
          NS_LOG_INFO (" ALLOCATION for RBG " << currentRB << " of " << *(availableRBGs.end())+currentRB);
          ...
          for (std::set<LteFlowId_t>::iterator it=itCurrentGroup->second.begin (); it!=itCurrentGroup->second.end (); it++)
            {
              ...
              if ((m_ffrSapProvider->IsDlRbgAvailableForUe (currentRB, flowId.m_rnti)) == false)
                {
                  NS_LOG_INFO("调用(m_ffrSapProvider->IsDlRbgAvailableForUe函数！！！！！！！！！！！！！！1");
                  continue;
                }
              ...
              if ( m_CqaMetric.compare ("CqaFf") == 0)
                ...
                {
                  EpsBearer bear;
                  uint8_t curqci = bear.GetPriority((*itStats).second.qci);
                  double curErrRate = bear.GetPacketErrorLossRate(curqci);
                  uint16_t curDelayMs = bear.GetPacketDelayBudgetMs(curqci);
                  metric = tbr_weight*pf_weight*hol;
                  NS_LOG_INFO (this << " RNTI " << (*itStats).first << " QCI " << +(*itStats).second.qci << " Priority " << +curqci << " PacketErrorLossRate " << curErrRate
                  << " PacketDelayBudgetMs " << curDelayMs << " achievableRate " << achievableRate << " avgThr " << (*itStats).second.lastAveragedThroughput << " metric " << metric
                  <<" HOL "<<hol);
                  NS_LOG_INFO (" metric "<<metric);
          ...
          // erase current RBG from the list of available RBG
          NS_LOG_INFO (" Allocated RBs:" << currentRB  <<" for RNTI "<< userWithMaximumMetric.m_rnti);
          availableRBGs.erase (currentRB);
