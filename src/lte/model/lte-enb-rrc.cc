...
{
  NS_LOG_FUNCTION (this <<"RNTI"<< (uint32_t) m_rnti);
  //建立无线承载LteDataRadioBearerInfo对象drbInf内存放EPC承载的参数
  ...
  NS_LOG_FUNCTION ("drbid="<<+drbid<<"lcid="<<+lcid<<"bid"<<+bid<<"传入的bearerId"<<+bearerId);
  ...
  NS_LOG_FUNCTION ("根据承载选择RLC模式");
  ObjectFactory rlcObjectFactory;
  rlcObjectFactory.SetTypeId (rlcTypeId);
  Ptr<LteRlc> rlc = rlcObjectFactory.Create ()->GetObject<LteRlc> ();
  rlc->SetLteMacSapProvider (m_rrc->m_macSapProvider);
  //MacSapProvider RLC数据传递到MAC
  rlc->SetRnti (m_rnti);
  ...
  //创建PDCP  RlcSapProvider PDCP给RLC在RLC实现  RlcSapUser在PDCP实现  AM/UM模式下用SIB1
  ...
  NS_LOG_FUNCTION ("EPC承载优先级"<< +drbInfo->m_epsBearer.GetPriority()<<"延迟"<<+drbInfo->m_epsBearer.GetPacketDelayBudgetMs()
                    <<"丢包率"<<+drbInfo->m_epsBearer.GetPacketErrorLossRate()<<"逻辑信道优先级"<<+drbInfo->m_logicalChannelConfig.priority
                    <<"EPS承载ID"<<+drbInfo->m_epsBearerIdentity);
...
UeManager::ScheduleRrcConnectionReconfiguration ()
{
  ...
  switch (m_state)
    ...
    case CONNECTED_NORMALLY:
      {
        NS_LOG_FUNCTION("CONNECTED_NORMALLY");
        m_pendingRrcConnectionReconfiguration = false;
...
{
  NS_LOG_FUNCTION (this << p << (uint16_t) bid << p->GetSize());
  switch (m_state)
    {
    case INITIAL_RANDOM_ACCESS:
    case CONNECTION_SETUP:
        ...
        if (it != m_drbMap.end ())
          {
            ...
            if (bearerInfo != NULL)
              {
                NS_LOG_FUNCTION ("EPC承载优先级"<< +bearerInfo->m_epsBearer.GetPriority()<<"延迟"<<+bearerInfo->m_epsBearer.GetPacketDelayBudgetMs()
                  <<"丢包率"<<+bearerInfo->m_epsBearer.GetPacketErrorLossRate()<<"逻辑信道优先级"<<+bearerInfo->m_logicalChannelConfig.priority
                  <<"EPS承载ID"<<+bearerInfo->m_epsBearerIdentity);
...
UeManager::AddDataRadioBearerInfo (Ptr<LteDataRadioBearerInfo> drbInfo)
{
  ...
  for (int drbid = (m_lastAllocatedDrbid + 1) % MAX_DRB_ID; drbid != m_lastAllocatedDrbid;  drbid = (drbid + 1) % MAX_DRB_ID)
    {
      NS_LOG_FUNCTION ("drbid"<<drbid);
...
LteEnbRrc::SendData (Ptr<Packet> packet)
{
  ...
  NS_LOG_FUNCTION("发送数据包 RNTI="<<tag.GetRnti());
...
{
  NS_LOG_FUNCTION ("EPC通过EpcEnbS1SapUser接口传递数据，在RRC侧执行无线承载建立");
